...
- `postgres` (3-replica streaming replication) and `kafka` (3-broker KRaft) both carry a **one-time manual step** at the top of their respective YAML files that's required the first time you apply the new config against an already-running single-node instance — read those comments before running `kubectl apply -k k8s` on an existing cluster.
- `redis` is a StatefulSet with 3 replicas for pod-count/restart-resilience testing — see the warning comment in `redis.yaml`: they are independent, non-clustered instances, not safe to rely on for session/checkpoint consistency yet.
