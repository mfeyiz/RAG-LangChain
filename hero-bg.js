/* ============================================================
   Hero Odyssey ambient background — WebGL lightning shader.
   Drives the fixed .hero-bg-canvas behind the whole app.
   Ported from the hero-odyssey component, tuned down (lower
   intensity / opacity) so it stays a subtle backdrop behind
   readable UI. No dependencies.
   ============================================================ */
(function () {
  "use strict";
  ...
  function initLightning(canvas, opts) {
    ...
    var lastFrame = 0;
    function render(now) {
      // throttle to ~30fps — plenty for a background, saves battery
  ...
  function boot() {
    ...
    if (!canvas) return;
    // Respect users who prefer reduced motion — leave a static gradient only.
    if (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    initLightning(canvas, { hue: 220, xOffset: 0, speed: 1.1, intensity: 0.42, size: 2 });
  }
